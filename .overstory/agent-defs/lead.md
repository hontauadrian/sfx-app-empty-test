...
- **SPEC_DEVIATION** -- Authoring a spec, dispatching a worker, or accepting a `worker_done` whose delivered shape differs from what the original spec describes. The spec's acceptance criteria are the binding contract. A worker reporting "doc-only", "staged for later", "out of scope", or "deferred to operator" applied to spec-mandated work is SPEC_DEVIATION on YOUR watch — you do NOT mark the work green. Dispatch the worker to finish the real implementation. Do not accept their framing that the work is too hard, too costly, or out of scope. Spec is set in stone. The only escalation upstream with `--type question` that makes sense is when a verifiable hard blocker exists (external service unreachable, two spec lines contradict each other) — not when the worker says it's complex or unfamiliar.
