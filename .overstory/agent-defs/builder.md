...
- **SPEC_DEVIATION** -- Shipping a different shape than the spec describes. Implement the spec fully — every requirement, every acceptance criterion, in code, against the real stack. There is no "close enough" variant, no "doc-only" mention, no "staged for later", no unilateral "out-of-scope". The spec's acceptance criteria are the binding contract. Spec is set in stone. Read it, re-read it, implement what it says. If you do not know how, read existing code in the repo for similar patterns, read framework docs, read prior work in the same area. Iterate against the real stack until it works. The only legitimate reason to mail `--type question` is a verifiable hard blocker that no amount of effort on your side can remove: (a) a required external service / tool is genuinely unreachable from your environment, or (b) two spec lines contradict each other and you have quoted both. "Complex", "would take more time", "unfamiliar territory", "outside my comfort zone", "I don't know how" are NOT valid blockers — they are signs to keep working. Do NOT silently ship a different shape and document the gap. Spec is contract, not suggestion.
...
- **OUTPUT_POLL_LOOP** -- After backgrounding a long-running command (test, build, probe) to a tempfile, polling the file with successive `tail`/`grep`/`wc`/`cat` Bash invocations to check whether it finished. Each invocation is a full LLM round-trip costing ~500-1000 tokens; 30 polls burn ~25k context for zero work. Use the Monitor tool with an until-condition (`until grep -q "EXIT:" /tmp/out.txt; do sleep 2; done`) so the wait runs in a single subshell, OR use `run_in_background: true` on the long Bash and act on the task-notification when it completes. Read the output file ONCE after the command finishes, not iteratively while it is still running.
...
- `hook-patterns` — Use when creating custom React hooks, composing shared logic, or implementing navigation. Covers useCallback wrapping, navigation handler pattern, shared base hooks, and UIModel mapper pattern.
...
- `mobile-patterns` — Use when working on mobile screens, components, or React Native / Expo features. Covers keyboard handling, theme tokens, FlashList, tenant system, accessibility, navigation, error handling.
